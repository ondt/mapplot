...
impl ImageMap {
	...
	/// TODO
	#[must_use]
	pub fn export_png(&self, scale: usize) -> Vec<u8> {
		...
		let pixmap_size = rtree.svg_node().view_box.rect.to_screen_size();
		let r = rtree.svg_node().view_box.rect;
		dbg!(r, pixmap_size);
		// let pixmap_size = ScreenSize::new(r)
		
		eprintln!("pixmap_size = {:#?}", pixmap_size);
