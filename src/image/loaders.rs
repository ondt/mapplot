...
pub trait TilesetLoader: Debug {
	...
	async fn download(&self, zoom: u8, x: u32, y: u32) -> Result<&[u8], TilesetLoaderError>;
...
#[derive(Debug, Clone)]
pub struct MaptilerTilesetLoader {
	tileset: String,
	format: String,
	hires: bool,
	apikey: String,
...
impl MaptilerTilesetLoader {
	#[must_use]
	pub fn new(tileset: impl AsRef<str>, format: impl AsRef<str>, hires: bool, apikey: impl AsRef<str>) -> Self {
		MaptilerTilesetLoader {
...
#[async_trait]
impl TilesetLoader for MaptilerTilesetLoader {
	async fn download(&self, zoom: u8, x: u32, y: u32) -> Result<&[u8], TilesetLoaderError> {
		eprintln!("downloading tile: {}/{}/{}", zoom, x, y);
		
		let url = format!("https://api.maptiler.com/maps/{tileset}/256/{zoom}/{x}/{y}{hires}.{format}?key={apikey}", tileset = self.tileset, zoom = zoom, x = x, y = y, hires = if self.hires { "@2x" } else { "" }, format = self.format, apikey = self.apikey);
